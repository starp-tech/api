...
export const createSyncGatewayUser = async (env: Env, user: UserInfo) => {
  const userId = user.user_id?.toLowerCase();
...
export const createUser = async (env: Env, user: UserInfo) => {
  ...
  const body = "roles=" + role + "&name=&groups=&password=" + password;
...
export const createCollection = async (
  ...
  env.PUBLIC_PARTY_AUTH = hash;
  const sql =
    "CREATE COLLECTION `" + scopeName.toLowerCase() + "`._default._default";
