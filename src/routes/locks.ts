...
export const cloudAuth = async (env: Env) => {
  ...
  const res = await fetch(env.TT_LOCK_API_URL + cloudApiURL, {
    ...
    body: `client_id=${payload.client_id}&client_secret=${payload.client_secret}&password=${payload.password}&username=${payload.username}`,
  });
  const data = res.json();
...
const cloudRegister = async (env: Env) => {
  ...
  const res = await fetch(env.TT_LOCK_API_URL + cloudRegisterURL, {
    method: "post",
    headers: { "Content-Type": contentType },
    body: `clientId=${data.clientId}&clientSecret=${data.clientSecret}&username=${data.username}&password=${originalPassword}&date=${data.date}`,
  });
  return res.text();
};

export const getLockList = async (request: Request, env: Env) => {
  ...
  const data = await Promise.all(
    (await res.json()).list.map(async (lock) => {
...
export const setNewKey = async (request: Request, env: Env) => {
...
export const setLockCard = async (request: Request, env: Env) => {
  const u = new URL(request.url);
  const lockId = u.searchParams.get("lockId");
...
export const clearLockCards = async (request: Request, env: Env) => {
  const u = new URL(request.url);
  const lockId = u.searchParams.get("lockId");
  const auth = await cloudAuth(env);
  console.info("clearLockCards", lockId);
...
export const setLockPassForStartEndTime = async (
  ...
  const res = await fetch(env.TT_LOCK_API_URL + addPassCodeUrl, {
    ...
    body: `clientId=${env.TT_LOCK_API_ID}&accessToken=${
      auth.access_token
    }&lockId=${lockId}&keyboardPwd=${passcode}&startDate=${startDate}&addType=2&endDate=${endDate}&date=${new Date().valueOf()}`,
  });
...
export const setLockCardForStartEndTime = async (
  ...
  const res = await fetch(env.TT_LOCK_API_URL + addCardUrl, {
    ...
    body: `clientId=${env.TT_LOCK_API_ID}&accessToken=${
      auth.access_token
    }&lockId=${lockId}&cardNumber=${cardId}&startDate=${startDate}&cardName=${cardName}&addType=2&endDate=${endDate}&date=${new Date().valueOf()}`,
  });
  const data = await res.json();
  return data;
};
...
const transferLock = async (env: Env, lockId, email) => {
  ...
  const res = await fetch(env.TT_LOCK_API_URL + transferLockUrl, {
    ...
    body: `clientId=${env.TT_LOCK_API_ID}&accessToken=${
      auth.access_token
    }&lockId=${lockId}&receiverUsername=${email}&lockIdList=[${lockId}]&date=${new Date().valueOf()}`,
  });
  const data = await res.json();
  return data;
};
